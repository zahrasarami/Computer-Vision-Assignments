function outputImage = myImResizeChessBoard( sourceImage , resizeFactor)
    ...
    for x=1:colemnD
        for y=1:rowD
            ...
            c = round(PColmn) ;
            r = round(PRow) ;
            d1 = max( abs(PColmn-c) , abs(PRow-r) );
            d2 = max( abs(PColmn-abs(c-1)) , abs(PRow-abs(r-1)) ) ;
            d3 = max( abs(PColmn-abs(c-1)) + abs(PRow-r) );
            d4 = max( abs(PColmn-c) + abs(PRow-abs(r-1)) );
